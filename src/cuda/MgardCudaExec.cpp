...
int launch_compress(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                    ...
                    enum mgard_cuda::error_bound_type mode, int lossless, bool verbose) {
...
int launch_decompress(const char * input_file, const char * output_file,
  ...
  std::vector<mgard_cuda::SIZE> shape = mgard_cuda::infer_shape(compressed_data, compressed_size);
  mgard_cuda::data_type dtype = mgard_cuda::infer_type(compressed_data, compressed_size);
  size_t original_size = 1;
...
bool try_compression(int argc, char *argv[]) {
  ...
  if (lossless_level == 0) {
    ...
    std::cout << mgard_cuda::log::log_info << "lossless: Huffman@GPU + LZ4@GPU\n";
