...
template <typename T>
int launch_decompress(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                      std::vector<mgard_cuda::SIZE> shape, T tol, T s, 
                      enum mgard_cuda::error_bound_type mode, bool verbose);
template <typename T>
int launch_compress(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                    std::vector<mgard_cuda::SIZE> shape, T tol, T s, 
                    enum mgard_cuda::error_bound_type mode, bool verbose);
...
std::vector<mgard_cuda::SIZE> get_arg_dims(int argc, char *argv[], std::string option) {
...
template <typename T>
void print_statistics(double s, enum mgard_cuda::error_bound_type mode, size_t n, T * original_data, T * decompressed_data) {
  std::cout << std::scientific;
  if (s == std::numeric_limits<T>::infinity()) {
    if (mode == mgard_cuda::ABS) {
      std::cout << mgard_cuda::log::log_info << "Absoluate L_inf error: " << mgard_cuda::L_inf_error(n, original_data, decompressed_data, mode) << "\n";
    } else if (mode == mgard_cuda::REL) {
      std::cout << mgard_cuda::log::log_info << "Relative L_inf error: " << mgard_cuda::L_inf_error(n, original_data, decompressed_data, mode) << "\n";
    }
  } else {
    if (mode == mgard_cuda::ABS) {
      std::cout << mgard_cuda::log::log_info << "Absoluate L_2 error: " << mgard_cuda::L_2_error(n, original_data, decompressed_data, mode) << "\n";
    } else if (mode == mgard_cuda::REL) {
      std::cout << mgard_cuda::log::log_info << "Relative L_2 error: " << mgard_cuda::L_2_error(n, original_data, decompressed_data, mode) << "\n";
    }
  }
  ...
  std::cout << mgard_cuda::log::log_info << "PSNR: " << mgard_cuda::PSNR(n, original_data, decompressed_data) << "\n";
}
...
int launch_compress(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
  ...
  if (D == 1) {
    ...
    if (verbose) start = high_resolution_clock::now();
    mgard::CompressedDataset<1, T> compressed_dataset = mgard::compress(hierarchy, original_data, s, tol);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall compression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 2) {
    ...
    if (verbose) start = high_resolution_clock::now();
    mgard::CompressedDataset<2, T> compressed_dataset = mgard::compress(hierarchy, original_data, s, tol);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall compression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 3) {
    ...
    if (verbose) start = high_resolution_clock::now();
    mgard::CompressedDataset<3, T> compressed_dataset = mgard::compress(hierarchy, original_data, s, tol);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall compression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 4) {
    ...
    if (verbose) start = high_resolution_clock::now();
    mgard::CompressedDataset<4, T> compressed_dataset = mgard::compress(hierarchy, original_data, s, tol);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall compression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 5) {
    ...
    if (verbose) start = high_resolution_clock::now();
    mgard::CompressedDataset<5, T> compressed_dataset = mgard::compress(hierarchy, original_data, s, tol);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall compression time: " << time_span.count() <<" s (" <<
  ...
  if (verbose) {
    ...
    launch_decompress<T>(D, dtype, output_file, temp, shape, tol, s, mode, true);
    readfile(temp, decompressed_data);
    print_statistics<T>(s, mode, original_size, original_data, (T*)decompressed_data);
...
int launch_decompress(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
  ...
  size_t original_size = 1;
  for (mgard_cuda::DIM i = 0; i < shape.size(); i++) original_size *= shape[i];
  ...
  if (D == 1) {
    ...
    std::copy(shape.begin(), shape.end(), array_shape.begin());
    const mgard::TensorMeshHierarchy<1, T> hierarchy(array_shape);
    mgard::CompressedDataset<1, T> compressed_dataset(hierarchy, s,
                    tol, compressed_data, compressed_size);
    if (verbose) start = high_resolution_clock::now();
    mgard::DecompressedDataset<1, T> decompressed_dataset = mgard::decompress(compressed_dataset);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall decompression time: " << time_span.count() <<" s (" <<
        (double)(original_size * sizeof(T))/time_span.count()/1e9 << " GB/s)\n";
    }
  ...
  if (D == 2) {
    ...
    std::copy(shape.begin(), shape.end(), array_shape.begin());
    const mgard::TensorMeshHierarchy<2, T> hierarchy(array_shape);
    mgard::CompressedDataset<2, T> compressed_dataset(hierarchy, s,
                    tol, compressed_data, compressed_size);
    if (verbose) start = high_resolution_clock::now();
    mgard::DecompressedDataset<2, T> decompressed_dataset = mgard::decompress(compressed_dataset);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall decompression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 3) {
    ...
    std::copy(shape.begin(), shape.end(), array_shape.begin());
    const mgard::TensorMeshHierarchy<3, T> hierarchy(array_shape);
    mgard::CompressedDataset<3, T> compressed_dataset(hierarchy, s,
                    tol, compressed_data, compressed_size);
    if (verbose) start = high_resolution_clock::now();
    mgard::DecompressedDataset<3, T> decompressed_dataset = mgard::decompress(compressed_dataset);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall decompression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 4) {
    ...
    std::copy(shape.begin(), shape.end(), array_shape.begin());
    const mgard::TensorMeshHierarchy<4, T> hierarchy(array_shape);
    mgard::CompressedDataset<4, T> compressed_dataset(hierarchy, s,
                    tol, compressed_data, compressed_size);
    if (verbose) start = high_resolution_clock::now();
    mgard::DecompressedDataset<4, T> decompressed_dataset = mgard::decompress(compressed_dataset);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall decompression time: " << time_span.count() <<" s (" <<
  ...
  if (D == 5) {
    ...
    std::copy(shape.begin(), shape.end(), array_shape.begin());
    const mgard::TensorMeshHierarchy<5, T> hierarchy(array_shape);
    mgard::CompressedDataset<5, T> compressed_dataset(hierarchy, s,
                    tol, compressed_data, compressed_size);
    if (verbose) start = high_resolution_clock::now();
    mgard::DecompressedDataset<5, T> decompressed_dataset = mgard::decompress(compressed_dataset);
    if (verbose) {
      end = high_resolution_clock::now();
      time_span = duration_cast<duration<double>>(end - start);
      std::cout << mgard_cuda::log::log_time << "Overall decompression time: " << time_span.count() <<" s (" <<
...
template int launch_compress<float>(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                      std::vector<mgard_cuda::SIZE> shape, float tol, float s, 
                      enum mgard_cuda::error_bound_type mode, bool verbose);
template int launch_compress<double>(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                      std::vector<mgard_cuda::SIZE> shape, double tol, double s, 
                      enum mgard_cuda::error_bound_type mode, bool verbose);

template int launch_decompress<float>(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                      std::vector<mgard_cuda::SIZE> shape, float tol, float s, 
                      enum mgard_cuda::error_bound_type mode, bool verbose);
template int launch_decompress<double>(mgard_cuda::DIM D, enum mgard_cuda::data_type dtype, const char * input_file, const char * output_file, 
                      std::vector<mgard_cuda::SIZE> shape, double tol, double s, 
                      enum mgard_cuda::error_bound_type mode, bool verbose);

bool try_compression(int argc, char *argv[]) {
  if (!has_arg(argc, argv, "-z")) return false;
  ...
  for (mgard_cuda::DIM d = 0; d < shape.size(); d++) shape_string = shape_string + std::to_string(shape[d]) + " ";
  ...
  else print_usage_message("wrong error bound mode.");
  ...
  if (dtype == mgard_cuda::data_type::Double)
    launch_compress<double>(D, dtype, input_file.c_str(), output_file.c_str(), shape, (double)tol, (double)s, mode, verbose);
  else if (dtype == mgard_cuda::data_type::Float)
    launch_compress<float>(D, dtype, input_file.c_str(), output_file.c_str(), shape, (float)tol, (float)s, mode, verbose);
  return true;
}


bool try_decompression(int argc, char *argv[]) {
  if (!has_arg(argc, argv, "-x")) return false;
  ...
  if (verbose) std::cout << mgard_cuda::log::log_info << "Verbose: enabled\n";
  if (dtype == mgard_cuda::data_type::Double)
    launch_decompress<double>(D, dtype, input_file.c_str(), output_file.c_str(), shape, (double)tol, (double)s, mode, verbose);
  else if (dtype == mgard_cuda::data_type::Float)
    launch_decompress<float>(D, dtype, input_file.c_str(), output_file.c_str(), shape, (float)tol, (float)s, mode, verbose);
  return true;
}
