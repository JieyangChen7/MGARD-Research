...
void * cast_to_non_const(const void * const_ptr) {
  const unsigned long long int const_address = (unsigned long long int)const_ptr;
...
#define KERNELS(T)                                                             \
  template bool compare_matrix<T>(SIZE nrow, SIZE ncol, T *v1, SIZE ldv1, T *v2,  \
                                  SIZE ldv2);                                   \
  template bool compare_matrix_cuda<T>(SIZE nrow, SIZE ncol, T *dv1, SIZE lddv1,  \
                                       T *dv2, SIZE lddv2);                     \
  ...
  template bool compare_matrix_cuda<T>(SIZE nrow, SIZE ncol, SIZE nfib, T *dv1,   \
                                       SIZE lddv11, SIZE lddv12, SIZE sizex1,     \
                                       T *dv2, SIZE lddv21, SIZE lddv22,         \
                                       SIZE sizex2, bool print_matrix);         \
  template void verify_matrix<T>(SIZE nrow, SIZE ncol, SIZE nfib, T *v, SIZE ldv1, \
                                 SIZE ldv2, std::string file_prefix, bool save, \
                                 bool verify);                                 \
  template void verify_matrix_cuda<T>(                                         \
      SIZE nrow, SIZE ncol, SIZE nfib, T *dv, SIZE lddv1, SIZE lddv2, SIZE sizex,    \
...
#define KERNELS(T)                                                             \
  ...
  template void print_matrix<T>(SIZE nrow, SIZE ncol, SIZE nfib, T *v, SIZE ldv1,  \
                                SIZE ldv2);
...
#define KERNELS(D, T)    \
  template void cudaMallocHelper<D, T>(Handle<D, T> &handle, void **devPtr, size_t size); \
  template void cudaMallocPitchHelper<D, T>(Handle<D, T> &handle, void **devPtr, size_t *pitch, size_t width,\
                           size_t height); \
  template void cudaMalloc3DHelper<D, T>(Handle<D, T> &handle, void **devPtr, size_t *pitch, size_t width, \
                        size_t height, size_t depth); \
  ...
  template void cudaMemcpy3DAsyncHelper<D, T>(                                 \
      ...
      size_t dheight, const void *src, size_t spitch, size_t swidth, size_t sheight, \
  ...
  template void cudaMemcpy3DPeerAsyncHelper<D, T>(                             \
      ...
      size_t dwidth, size_t dheight, const void *src, int src_dev, size_t spitch,    \
...
#define KERNELS(D, T)    \
  template void PrintSubarray<SubArray<D, T>>(std::string name, SubArray<D, T> subArray);
