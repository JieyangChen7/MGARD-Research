...
int main(int argc, char *argv[]) {
  ...
  if (D == 1) {
    if (device == 0) {
      ...
      mgard::CompressedDataset<1, float> compressed_dataset = mgard::compress(hierarchy, in_buff, s, data_L_inf_norm * tol);
      out_size = compressed_dataset.size();
      mgard::DecompressedDataset<1, float> decompressed_dataset = mgard::decompress(compressed_dataset);
      memcpy(mgard_out_buff, decompressed_dataset.data(), num_float * sizeof(float));
    } else {
    ...
    if (device == 0) {
      const mgard::TensorMeshHierarchy<2, float> hierarchy({shape[1], shape[0]});
      mgard::CompressedDataset<2, float> compressed_dataset = mgard::compress(hierarchy, in_buff, s, data_L_inf_norm * tol);
      out_size = compressed_dataset.size();
      mgard::DecompressedDataset<2, float> decompressed_dataset = mgard::decompress(compressed_dataset);
      memcpy(mgard_out_buff, decompressed_dataset.data(), num_float * sizeof(float));
    } else {
    ...
    if (device == 0) {
      const mgard::TensorMeshHierarchy<3, float> hierarchy({shape[2], shape[1], shape[0]});
      mgard::CompressedDataset<3, float> compressed_dataset = mgard::compress(hierarchy, in_buff, s, data_L_inf_norm * tol);
      out_size = compressed_dataset.size();
      mgard::DecompressedDataset<3, float> decompressed_dataset = mgard::decompress(compressed_dataset);
      memcpy(mgard_out_buff, decompressed_dataset.data(), num_float * sizeof(float));
    } else {
    ...
    if (device == 0) {
      const mgard::TensorMeshHierarchy<4, float> hierarchy({shape[3], shape[2], shape[1], shape[0]});
      mgard::CompressedDataset<4, float> compressed_dataset = mgard::compress(hierarchy, in_buff, s, data_L_inf_norm * tol);
      out_size = compressed_dataset.size();
      mgard::DecompressedDataset<4, float> decompressed_dataset = mgard::decompress(compressed_dataset);
      memcpy(mgard_out_buff, decompressed_dataset.data(), num_float * sizeof(float));
    } else {
