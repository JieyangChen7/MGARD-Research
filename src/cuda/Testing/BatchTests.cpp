...
template <typename T>
struct Result test(int D, T * original_data, enum data_type dtype, std::vector<mgard_cuda::SIZE> shape, enum device dev, double tol, double s, enum mgard_cuda::error_bound_type mode) {
...
void print_config(char * input_file, enum data_type dtype, std::vector<mgard_cuda::SIZE> shape, enum device dev, 
...
int main(int argc, char *argv[]) {
  ...
  for (int sp = 0; sp < shapes.size(); sp++) {
    for (int dt = 0; dt < dtypes.size(); dt++) {
      for (int ebt = 0; ebt < ebtypes.size(); ebt++) {
        for (int s = 0; s < ssd.size(); s++) {
          for (int tol = 0; tol < tols.size(); tol++) {
            struct Result result_cpu, result_gpu;
            if (dtypes[dt] == SINGLE) {
              size_t original_size = 1;
              for (int i = 0; i < shapes[sp].size(); i++) original_size *= shapes[sp][i];
              ...
              result_cpu = test<float>(shapes[sp].size(), 
                    original_data, dtypes[dt], shapes[sp], CPU, tols[tol], ssf[s], ebtypes[ebt]);
              result_gpu = test<float>(shapes[sp].size(), 
                    original_data, dtypes[dt], shapes[sp], GPU, tols[tol], ssf[s], ebtypes[ebt]);
              ...
              result_cpu = test<double>(shapes[sp].size(), 
                    original_data, dtypes[dt], shapes[sp], CPU, told[tol], ssd[s], ebtypes[ebt]);
              result_gpu = test<double>(shapes[sp].size(), 
                    original_data, dtypes[dt], shapes[sp], GPU, told[tol], ssd[s], ebtypes[ebt]);
              delete [] original_data;
            }

            print_config(input_file, dtypes[dt], shapes[sp], CPU, tols[tol], ssd[s], ebtypes[ebt]);
